...
export default async function CardPage({ params }) {
  ...
  if (!card) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-slate-100 font-sans text-center">
        ...
        <Link href="/" className="bg-violet-600 hover:bg-violet-700 text-white rounded-xl px-5 py-2.5 text-xs font-bold shadow-md hover:shadow-violet-500/10 active:scale-95 transition-all">
  ...
  CardService.recordView(hash).catch(() => {});

  return <CardVisitorView card={card} />;
}